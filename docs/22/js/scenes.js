...
class VanButtonMaker {
    static fromColumn(column, obj) {
        if ('van-button'!==column.type) { return obj }
        for (let [key, type] of Object.entries(VanButton.ATTRS)) {
            if (!obj.attrs.hasOwnProperty(key)) { continue }
            obj.attrs[key] = this.#boxing(type, obj.attrs[key])
        }
        return obj
        //return {tagName:obj., attrs:obj.attrs, datalist:datalist, children:null}
        /*
        for (let fn of ['onpush','onhold']) { obj[fn] = new Function(column[fn]) }
    ...
    static #boxing(type, value) { // value:string, type:型名(typeof)
        switch(type.toLowerCase()) {
            case 'undefined': return undefined
            case 'null': return null
            case 'object': return {}
            case 'array': return []
            case 'boolean': return (['true','1'].some(v=>v===value.toLowerCase()))
